...
function openDietMealPopup(ex, workoutId, exercises, isSupabaseLoaded, list, onClose, isEditable = false) {
  ...
  function renderItems() {
    ...
    if (currentItems.length === 0) {
      ...
      currentItems.forEach((itm, i) => {
        ...
        chip.innerHTML = `
          <span class="diet-food-num" style="align-self: flex-start; margin-top: 1px;">${i + 1}.</span>
          <div style="flex: 1; display: flex; flex-direction: column; gap: 4px; align-items: flex-start;">
            <span class="diet-food-name" style="line-height: 1.3; text-align: left;">${escapeHtml(itm.name)}</span>
            ${qty ? `<span class="diet-food-qty" style="margin-left: 0;">${escapeHtml(qty)}</span>` : ""}
          </div>
          ${isEditable 
  ...
  if (isEditable) {
    const saveBtn = panel.querySelector("#dietSaveCloseBtn");
    if (saveBtn) {
